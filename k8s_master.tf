resource "proxmox_vm_qemu" "k8s_master" {
  ...
  qemu_os                 = "l26"
  desc                    = "${var.vm_name_prefix}-master-${count.index + 2}, generated by Terraform Alx v${var.module_version}"
  disks {
    scsi {
      scsi0 {
        disk {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "echo '    server ${var.vm_name_prefix}-master-${count.index + 2} ${var.ip_address_start}.${var.master_node_ip_start + 1 + count.index}:6443 check' | sudo tee -a /etc/haproxy/haproxy.cfg",
